...
class Content extends React.Component<IContentProps, IContentState> {
  ...
  public onPreAnimate(time: number, advance: number, state: IContentAnimation = { width: 0, height: 0, applied: false, valid: false }): IContentAnimation {
    this._measure = this._measure.iterate(advance);
    const { width, height } = limitSize(this._measure.size);
    if (width !== state.width || height !== state.height) {
      const applied = false;
      const valid = true;
      state = { width, height, applied, valid };
  ...
  public componentDidMount() {
    this._measure = this._measure.iterate(0);
